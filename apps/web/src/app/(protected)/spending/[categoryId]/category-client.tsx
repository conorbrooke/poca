...
export function CategoryClient({ categoryId }: CategoryClientProps) {
  ...
  const loadCategoryMeta = useCallback(async () => {
    ...
    try {
      ...
      setIsVariableValue(Boolean(detailRes.category.isVariable));
    ...
    try {
      await apiFetch(`/spending/categories/${categoryId}`, {
        ...
        body: JSON.stringify({
          ...
          isEssential: kindValue === "EXPENSE" ? isEssentialValue : false,
          isVariable: kindValue === "EXPENSE" ? isVariableValue : false,
        }),
      });
      await loadData();
    } catch (err) {
